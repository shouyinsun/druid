...
public class Sequences
{
  ...
  /***
   *传入实现Iterable接口的对象,返回BaseSequence
     */
  ...
  /**
   *执行accumulate 方法时根据传入的 Predicate过滤
   * Predicate为true时进行累积
   * 为false时则抛弃
   * groupBy 的having就是filter方法实现的
     */
  ...
  public static <T> Sequence<T> withEffect(final Sequence <T> seq, final Runnable effect, final Executor exec)
  {
    ...
    /**
     *将sequence中的元素排序,先转化成list,然后list中排序,再转化成sequence
     * sequence太大,有内存风险
     */
