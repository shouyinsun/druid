...
/**
 * Broker查询的核心类
 * 根据时间线路由,分发查询请求到不同Segment,然后汇总请求,并将结果缓存起来
 */
  ...
  private class SpecificQueryRunnable<T>
  {
    ...
    Sequence<T> run(final UnaryOperator<TimelineLookup<String, ServerSelector>> timelineConverter)
    {
      ...
      return new LazySequence<>(() -> {
        ...
        //MergeSequence 把缓存中和DruidServer返回的Sequence合并
        return Sequences
    ...
    {
        //根据intervals在TimeLine中查找获取TimeLineObjectHolder列表
      ...
      //迭代TimeLineObjectHolder构建 Pair<ServerSelector, SegmentDescriptor> 集合
